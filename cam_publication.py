...
def attention_cmap():
    ...
    """Transparent below threshold, then warm and increasingly opaque. Read as
...
def main():
    ...
    for cp in args.ckpts:
        ...
        if args.sanity and not sanity:
            ...
            print(f" Sanitycheck: correlation between the real map and the map from "
                  f"randomised weights = {c:.3f} near 0 passes; near 1 means the map reflects image structure, not the model.")
    ...
    print("\n" + "-" * 40)
    print(f" Volumetric Attribution ({args.method})")
