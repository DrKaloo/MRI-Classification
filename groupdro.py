...
class GroupDRO(nn.Module):
    def __init__(self, base_loss_fn: nn.Module, num_classes: int,
        ...
        # clone the trainer's loss with reduction="none" so per-sample losses are available. Every other setting (gamma, class weights, label smoothing)
        ...
        # Exponentiated gradient saturates: with a persistent loss gap, q drifts to 1.0 on a single group and the model then trains on that group alone.
        # Mixing a floor of uniform mass back in each step bounds how far it can go, so no group is ever fully abandoned. Unit-tested: without this, q
    ...
    def forward(self, logits: torch.Tensor, y: torch.Tensor,
        ...
        # weight only the groups actually present; renormalise over them so the loss scale stays comparable to the baseline mean loss
